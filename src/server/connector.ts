...
/**
 * Implementation of the Office 365 Connector
 */
@ConnectorDeclaration(
    "/api/connector/connect",
    "/api/connector/ping"
)
@PreventIframe("/config.html")
export class MyConnector implements IConnector {
    ...
    public Ping(req: Request): Array<Promise <void>> {
        ...
        try {
            this.connectors.push("/connectors",
                (this.connectors.getData("/connectors") as IMyConnectorData[])
                    .filter(c => {
                        return c.existing;
                    }));
        } catch (error) {
        ...
        return (this.connectors.getData("/connectors") as IMyConnectorData[]).map((connector, index) => {
